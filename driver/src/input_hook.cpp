    ...
    double                   timeOffset)
{
    ...
    std::string dbgPath;
    {
        ...
        // Log first few forwarded bool changes for debugging
        static int s_fwdLog = 0;
        if (virtHandle != k_ulInvalidInputComponentHandle && value && s_fwdLog < 30) {
            ++s_fwdLog;
            auto hi = g_boolHandles.find(handle);
            if (hi != g_boolHandles.end()) dbgPath = hi->second.path;
        }
    }
    if (virtHandle != k_ulInvalidInputComponentHandle) {
        if (!dbgPath.empty())
            DLog("OVRDancers: InputFwd bool '%s'=%d\n", dbgPath.c_str(), (int)value);
